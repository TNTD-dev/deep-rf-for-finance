...
Usage:
    ...
    .venv/bin/python scripts/train_ddpg.py --total-timesteps 10000  # smoke
    .venv/bin/python scripts/train_ddpg.py --config configs/ddpg.yaml --seed 7
"""
...
def main() -> int:
    ...
    if result["aborted"]:
        print(
            "\nWARNING: DDPG aborted "
            f"({result.get('aborted_reason')}). "
            "Run 'scripts/train_ppo.py' for backup."
        )
        return 2
    return 0
